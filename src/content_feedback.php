...
* @version $Id: content_feedback.php 39701 2014-03-28 14:22:42Z weinert $
*/
  ...
  */
  function sendEmail($content = array()) {
    ...
    if (!empty($content['NAME'])) {
      $emailObj->setSender($content['FROM'], $content['NAME']);
    }
    $emailObj->setTemplate('subject', $this->data['msg_subject'], $content);
    $emailObj->setTemplate('body', $this->data['msg_body'], $content);
  ...
  function checkMailDialogInput() {
    ...
    if ($this->data['show_field_sender']) {
      if (empty($this->params['mail_name']) ||
          !PapayaFilterFactory::isNotEmpty($this->params['mail_name'], TRUE)) {
    ...
    if (empty($this->params['mail_subject']) ||
        !PapayaFilterFactory::isNotEmpty($this->params['mail_subject'], TRUE)) {
      $result['mail_subject'] = $this->data['cap_subject'];
    }
    if (empty($this->params['mail_message']) ||
        !PapayaFilterFactory::isNotEmpty($this->params['mail_message'], TRUE)) {
