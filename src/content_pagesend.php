...
* @version $Id: content_pagesend.php 39732 2014-04-08 15:34:45Z weinert $
*/
    ...
    if (isset($content) && is_array($content)) {
      ...
      if (isset($this->params['submit']) &&  $this->params['submit']) {
        ...
        if (!$errors) {
          ...
          if ($emailObj->send($this->params['mail_to'])) {
  ...
  */
  function checkMailDialogInput() {
    ...
    if (empty($this->data['cap_recipient']) ||
        !PapayaFilterFactory::isEmail($this->params['mail_to'], TRUE)) {
      $result['mail_to'] = $this->data['cap_recipient'];
    }
    if (empty($this->params['mail_from']) ||
        !PapayaFilterFactory::isEmail($this->params['mail_from'], TRUE)) {
